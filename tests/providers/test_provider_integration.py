...
def _request_rejected_response() -> ProviderResponse:
    return ProviderResponse(
        status_code=400,
        headers={"content-type": "application/json"},
        json_body={"error": {"message": "stop integration script"}},
        body_size=80,
    )


def _write_spec_response() -> ProviderResponse:
    return _action_response(
        {
            "type": "tool_call",
            "phase": "spec",
            ...
            "reason": "Create the task specification artifact.",
        }
    )
...
def test_fake_transport_provider_runs_through_agent_loop(
    ...
    transport = FakeTransport(
        [
            _write_spec_response(),
            _finish_phase_response("spec"),
            _request_rejected_response(),
        ]
    )
    ...
    assert state.artifacts["SPEC.md"] is True
    assert any(event.event_type == "tool_called" for event in result.trace_events)
...
def test_fake_transport_no_real_network_access(
    ...
    transport = FakeTransport(
        [_finish_phase_response("spec"), _request_rejected_response()]
    )
