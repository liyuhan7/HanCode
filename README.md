...
- Slash 命令：`/task <goal>`、`/tasks`、`/use <task-id>`、`/run`、`/resume`、`/status`、`/trace`、`/artifacts`、`/open <name>`、`/rollback`、`/clear`、`/help`、`/quit`。
...
- TUI 只通过应用服务（TaskService / InteractionService / InspectionService / RecoveryService）操作，不直接调用 AgentLoop、工具、state 或 trace 写入。
