...
PHASE_CONTRACTS: dict[Phase, str] = {
    Phase.SPEC: (
        "Understand the assignment and record it with record_requirements "
        "(goal + a requirements array, each with source_text, your own "
        "student_understanding, acceptance_evidence, priority, is_core). "
        "record_requirements renders SPEC.md deterministically; do not hand-write "
        "SPEC.md with write_file. "
        ...
        "the available evidence is sufficient, call record_requirements instead "
        "of continuing read-only exploration. "
        "Do not modify source code."
    ),
    Phase.PLAN: (
        "Use SPEC.md to create PLAN.md with record_plan: a decisions array "
        "(candidate options chosen_option/rejected_options with a rationale per "
        "decision) and a plan_steps array (description, requirement_refs, "
        "planned_paths, verification). "
        "record_plan renders PLAN.md; do not hand-write PLAN.md with write_file. "
        "Do not modify source code."
    ),
    Phase.CODE: (
...
"The remediation decision is persisted to test_remediation.json and "
        ...
        "Do not call record_review only while a TEST failure is active "
        "(sections.test_failure is present and latest_test_status is failed). "
        "A rejected write of a delivery artifact (context active_failure where "
        "tool is write_file) is not a test failure; it does not block record_review. "
        "Do not try to create REVIEW.md with write_file; it is generated only by "
        "the delivery pipeline. "
        "When the latest tests are passed, legacy tasks record requirement coverage "
        "with requirements/risks. Tasks that already have structured SPEC.md "
        "evidence must use record_review with requirement_reviews (requirement_id, "
        "change_refs, test_refs, status, risk), quality_findings, untested_risks, "
        "plan_deviations, and delivery_recommendation. "
        "Use only C-* IDs from sections.learning_evidence.changes for change_refs "
        "and T-* IDs from sections.learning_evidence.test_attempts for test_refs; "
        "file paths and mem-* IDs are invalid. "
        "The learning_evidence entries include the authoritative details behind "
        "those IDs. Do not search memory for C-* or T-* details. Make one evidence pass: "
        "inspect the supplied requirement, change, test, and diff evidence once, then "
        "call record_review instead of repeating read_file, get_diff, or memory_search. "
        "A successful record_review completes this phase. "
        "Do not rerun tests in REVIEW; fixes must return through CODE and TEST."
    ),
    Phase.DELIVER: (
        "Use record_knowledge and get_diff to produce the required knowledge and "
        "delivery evidence. Legacy tasks use record_knowledge.items. Tasks with "
        "structured SPEC.md evidence must use grounded record_knowledge.cards; "
        "each card must explain the problem, context, principle, solution, "
        "evidence_refs, applicability, common mistake, and transfer example. "
        "Use only existing IDs from sections.learning_evidence in evidence_refs; "
        "every card needs at least one concrete C-*, T-*, or F-* reference. File "
        "paths, hashes, trace event IDs, and mem-* IDs are invalid. "
