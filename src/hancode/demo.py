...
_PACKAGED_FIXTURE_ROOT = resources.files("hancode").joinpath("_demo_fixture")
...
def _copy_packaged_fixture(destination: Path) -> None:
    for relative_path in _FIXTURE_DIGESTS:
        source = _PACKAGED_FIXTURE_ROOT.joinpath(*relative_path.split("/"))
...
def run_packaged_mock_demo() -> DeliveryResult:
    ...
    with TemporaryDirectory(prefix="hancode-mock-demo-") as temporary_directory:
        ...
        _copy_packaged_fixture(project_root)
