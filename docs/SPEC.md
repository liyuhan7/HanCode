...
- Provider-facing system contract 必须显式声明 `task_context.user_interventions.effective` 的权威语义：当前 run 内持续有效；较大 sequence 覆盖冲突的较小 sequence；冲突时覆盖任务目标、旧计划和 observation，但不得覆盖 system rules、phase gates、ToolPolicy、Approval 或 Checkpoint。
...
- TUI composer 应始终接受用户文本；有 active task 且不在 `WAITING_INPUT` 时，普通文本作为当前 active run 的 Steering 持久化，并在下一个安全点生效，不依赖 TUI Worker 的 busy/PauseToken/request identity 快照。Worker 忙时不得启动第二个 mutation Worker；Worker 已结束但 run 仍有效时自动 resume。`WAITING_INPUT` 普通文本仍回答问题，`WAITING_APPROVAL` 普通文本作为 Steering 并自动 resume，但不得隐式 approve/reject；`PAUSED` 接受 Steering 但仍要求显式 `/resume`。
...
| 本地测试环境 | 执行测试命令 | 测试命令来自配置，或由 LLM 提供后经人工确认；只允许单条 argv，不允许通用 shell 语法。 |
...
```json
{
  ...
  "pending_checkpoint_recovery_id": null
}
```
...
- `pending_checkpoint_recovery_id` 仅在普通启动发现已变化的 pending checkpoint 时记录对应 ID；显式恢复成功或安全 abort 后清除；旧 state 缺失此字段按 `null` 兼容加载。
...
| `checkpoint_aborted` | pending checkpoint 经安全 abort 或显式恢复后退出 rollback 链。 |
| `checkpoint_recovery_required` | 普通启动发现 pending checkpoint 已发生源文件变化，要求显式 resume。 |
| `checkpoint_recovery_failed` | pending checkpoint 恢复或补偿失败。 |
| `checkpoint_abort_failed` | pending checkpoint 的 abort 状态持久化失败。 |
...
- 当前实现达到上限时以 `checkpoint_limit_exceeded` fail-closed 拒绝新增，不自动 pruning；自动清理仍是规划能力。
- checkpoint 的创建、清理、使用和 rollback 都必须写入 `trace.jsonl`。

Checkpoint manifest 的 `status` 只能是 `pending`、`committed`、`rolled_back` 或 `aborted`：

- `pending` 表示 before snapshot 已持久化，但源文件修改尚未提交；不得猜测为 committed。
- `committed` 必须有 after hash 且可 rollback；`rolled_back` 和 `aborted` 均关闭 `rollback_available`。
- 普通启动发现未修改 pending 时标记 `aborted`，不写业务文件，并将 `latest_checkpoint` 回指最近仍可 rollback 的 committed checkpoint。
- 普通启动发现 pending 对应源文件已变化时保存 `inconsistent=true`、`rollback_required=true` 和 `pending_checkpoint_recovery_id`，写入 `checkpoint_recovery_required` 后停止高风险动作。
- 只有显式 `resume=True` 才可以在所有 manifest、快照、hash、路径和 symlink/junction 预检通过后恢复 before snapshot；恢复成功后 state 进入 `blocked`，本次显式 resume 再转为 `running`。
- 预检或补偿失败必须 fail-closed，保持业务文件不写入或恢复为原状态，并返回结构化错误；`checkpoint_recovery_failed` / `checkpoint_abort_failed` 只记录脱敏摘要。

T21-R1 暴露 `abort_pending_checkpoint(task_root, checkpoint_id, *, restore_files)` 与 `reconcile_pending_checkpoint(task_root, state, *, recover)`；`FilesystemStateStore.reconcile(recover_pending=False)` 将后者接入 AgentLoop，`resume=True` 是唯一的 pending 恢复授权。
...
- 如果 checkpoint manifest、身份、路径、快照、hash 或 symlink/junction 校验失败，相关高风险动作必须停止，且恢复流程不得写入业务文件。
- 如果 pending checkpoint 未发生源文件变化，启动对账可安全将其标记为 `aborted`；如果已发生变化，普通启动必须进入 `inconsistent + rollback_required`，只有显式 `resume=True` 可恢复经预检的 before snapshot。
...
- `edit_file(project_root, path, old_string, new_string)` 只允许 `old_string` 恰好匹配一次；空字符串、零次/多次匹配、无变化、越界、protected、目录和非 UTF-8 文件必须拒绝且保持原文件不变。原子替换成功返回 path、replacement count、bytes written，并设置 `mutation_applied=true`；可证明未写入的失败设置 `false`，替换阶段不确定时设置 `null`。
- `run_tests(project_root, command, *, runner=None, timeout_seconds=120.0)` 接收配置 fallback 或审批后的显式命令，使用 `shlex.split` 生成单条 argv，固定工作目录、`shell=False`、捕获输出和 `check=False`；缺少命令、非法 shell 语法、成功、非零退出、超时和启动异常均返回结构化 `ToolResult`。命令、stdout、stderr 在进入结果前脱敏。
- `build_default_tool_registry(config, *, run_tests_tool=None)` 必须装配 `read_file`、`list_files`、`search_text`、`write_file`、`edit_file`、`run_tests` 六个工具；未注册工具仍 fail-closed，rollback 由 AgentLoop / CheckpointManager 处理。
...
- 当前实现超过 checkpoint 上限时返回结构化错误并拒绝创建，不执行自动清理。
- rollback 可以恢复最近 checkpoint。
- rollback 后记录 restored files。
- rollback 结果写入 trace 和 REVIEW。
- manifest 损坏时禁止继续 rollback 并返回错误。
- pending checkpoint 不得被猜测为 committed；未修改时安全 abort，已变化时普通启动 fail-closed，显式 `resume=True` 才能恢复校验通过的 before snapshot。
- abort / 恢复成功、要求显式恢复、恢复失败和持久化失败均须有结构化结果与可追溯 trace；恢复失败不得静默覆盖人工修改。
...
上表记录 SPEC 规划的完整命令矩阵；当前已交付并可执行的 headless CLI 以 README 和 `src/hancode/cli.py` 为准。`hancode run`、无参数 REPL/TUI、真实 Provider 调用和 Docker 仍未交付，不得在冷启动验证中当作可运行能力。
...
- 路径逃逸、绝对路径逃逸、符号链接逃逸。
- 执行未经配置或未经审批的测试命令，或执行通用 shell 命令。
...
| 是否提供 HITL 人工确认 | MVP 复用 ApprovalPolicy / ApprovalService；显式 `run_tests.command` 必须确认 | 不引入通用 shell；复杂多步骤审批仍不在本卡范围。 |
