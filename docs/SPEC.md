...
- `run_tests(project_root, command, *, runner=None, timeout_seconds=120.0)` 只接受配置命令，使用确定性的 argv、固定工作目录、`shell=False`、捕获输出和 `check=False`；缺少命令不启动进程，成功、非零退出、超时和启动异常均返回结构化 `ToolResult`。命令、stdout、stderr 在进入结果前脱敏。
...
- 测试命令只能来自项目配置，Action 不携带命令参数；`run_tests` 使用确定性 argv、固定 `cwd`、`shell=False`、捕获输出和 `check=False`，默认超时 120 秒，不提供通用 shell 或自动安装依赖。
- 测试命令、stdout 和 stderr 在写入 `ToolResult`、state 或 trace 前必须脱敏；缺少或为空的配置命令不得启动进程。
