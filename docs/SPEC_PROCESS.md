...
第一阶段新鲜评审进一步指出，仅检查关键词存在不足以保护文档承诺。T27 因此新增了按 README 分区的正反断言和 secret-like 文本扫描：当前可用命令区域不得承诺未实现能力，wheel 区域必须包含安装后的裸 CLI 命令。新增断言先得到 `1 failed、7 passed`，补充 `### wheel 安装后的命令` 标题后得到 `8 passed`；第二阶段评审和全量门禁仍待完成。
