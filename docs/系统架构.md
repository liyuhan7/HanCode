...
T17 当前提供函数式 `create_checkpoint(task_root, files, reason, *, created_at=None)` 与 `commit_checkpoint(task_root, checkpoint_id)`；T18 才实现实际恢复。创建函数在 code phase 对 SOURCE 区目标建立 `pending` manifest，现有文件保存 before snapshot/hash，不存在的 SOURCE 目标记录为 `action="create"`。提交函数验证同 task 的 manifest、before snapshot/hash 和当前目标文件后，补 after hash 并原子更新为 `committed`。
...
T17 不实现 rollback、checkpoint pruning、AgentLoop/FileTools 自动调度、跨进程锁、TOCTOU 消除或 pending crash reconcile；这些分别属于 T18、T21 或后续持久化增强。
