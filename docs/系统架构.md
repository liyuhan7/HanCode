...
| v2.1 | 2026-08-06 | S17-TUI 运行中 Steering 入口：busy 状态普通文本路由为 `STEER`，UI 线程通过 `InterventionService` 写入当前 `active_run_id` 的 InterventionStore，不启动第二个 mutation Worker；`WAITING_INPUT` 和 `WAITING_APPROVAL` 既有优先级保持不变；Store 路径锁与 AgentLoop 下一轮 Snapshot 形成 UI-to-worker 数据通道。 |
...
9. TUI 运行中入口（S17-TUI/R4）：当当前 mutation Worker 处于 busy 状态且 active/running task identity 匹配时，普通文本被分类为 `STEER`，由 UI 线程同步调用 `InterventionService.submit` 写入当前 run 的 `interventions.jsonl`；不创建第二个 mutation Worker，成功后提示下一安全点生效。`WAITING_INPUT` 仍优先作为回答；`WAITING_APPROVAL` 普通文本写入 Steering 并自动 resume，但 `/approve`/`/reject` 仍必须显式输入；`PAUSED` 不自动恢复。
