...
3. 校验每个 committed checkpoint 的 before_snapshot 文件存在。【未实现 / post-MVP】
4. 处理 pending checkpoint：回滚并标记 aborted（见 §6.9.1）。【未实现 / post-MVP】
5. 若 state.json 损坏或缺字段，进入 blocked 或 failed，提示人工修复，不从 history.jsonl 静默重建状态。
6. reconcile 结果写 trace（state_reconciled）；若存在漂移，同时写 state_inconsistent，记录漂移项和被禁止的高风险动作。
```

> 实现现状（T21 / T21-R1）：`reconcile_state()` 仅落地步骤 1-2（schema 校验 + artifact 漂移检测→`inconsistent`）与步骤 5-6（损坏/缺字段 fail-closed、写 `state_reconciled` / `state_inconsistent` trace）。步骤 3-4 的 committed checkpoint 快照校验与 pending checkpoint 自动回滚标记 aborted **尚未实现**，留待后续任务；当前 pending checkpoint 的处置依赖 T21 既有 `resume` 通道与 `rollback_required` 标记，不在启动 reconcile 中自动猜测提交或回滚。
