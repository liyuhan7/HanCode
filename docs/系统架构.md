...
| review  | `read_file`, `list_files`, `search_text`, `run_tests`, `rollback_last_checkpoint`, `write_file`, `write_report` |
...
导出模块只复制 state 声明的六类交付 artifact，allow-list 不包含 `memory/`；因此 Memory 不会进入
交付目录。S13-R5 的离线 MockLLM Demo 通过“初始读取 → Memory Tool → 新写入 stale → 测试失败与
