...
`ConfigLoader` 负责加载 HanCode 运行所需的项目配置、任务配置、默认配置和凭据状态。

职责：

1. 加载 `.hancode/project.json`。
2. 加载 task 级 `state.json`。
3. 加载默认配置，例如 `max_steps`、`retry_budget`、protected patterns。
4. 加载 LLM provider、模型名称和模型参数。
5. 加载测试命令和构建命令。
6. 加载 workspace root、task workspace 路径和 checkpoint 路径。
7. 读取凭据配置状态，但不回显明文凭据。
8. 加载交互模式、写入根目录、observation 截断阈值和 checkpoint 保留策略。
9. 将配置对象注入 AgentLoop、ContextBuilder、ToolPolicy、ToolExecutor、CheckpointManager 和 TUI。
...
交付与验证要求属于 MVP 的课程项目提交能力。`python -m build` 应能生成 wheel / sdist，CI 应默认运行 MockLLM 核心机制测试；二者都不得依赖真实 LLM、真实 API key 或网络。
...
10. `python -m build` 可以生成 wheel / sdist。
...
| Package Manager | pip / npm / mvn                         |    可选 | 依赖安装或测试                    | 供应链风险、耗时         | MVP 不自动安装依赖                   |
...
| `workspace_root`     | project.json       | 限制路径范围       |
