...
| v1.4 | 2026-07-21 | S4 统一交付流程：新增 CheckpointQueryRepository、ChangeInspectionService、CheckpointInspectionService、DeliveryInspectionService、BuildService；新增 `get_diff`/`run_build`/`read_test_report`/`list_checkpoints`/`record_review`/`record_knowledge` 工具；新增 DeliveryPipeline、DeliveryEvidenceStore 和统一 DeliveryService；新增 ToolSpec 单一真源；DemoRunner 收敛到统一交付路径。 |
| v1.5 | 2026-07-23 | S5 TUI 产品化：统一 TUI Operation Worker、Inspection ViewModel/Detail 路由、Approval/Rollback Modal、Export/Build 操作、启动 Trace 恢复和宽中窄响应式布局；完整路径由 Textual `run_test`、MockLLM Demo 和打包门禁验证。 |
| v1.6 | 2026-07-29 | S7 中文开发者工作台：会话级深浅主题、中文文案、语义活动流、Focus/Inspect 中栏切换、结构化 Inspector、分类决策文案、Command Palette 与四 Tab 窄屏布局；仍只消费 Application Service 的只读结果，不改变 Harness Core。 |
| v1.7 | 2026-08-06 | S14 Deliver 学习发布契约：结构化学习证据成为权威源；新增 IMPLEMENTATION、历史测试尝试、Traceability、KnowledgeCard、generated/student 分区、Collect/Validate/Synthesize/Reflect/Publish 五步管线，以及 submission/learning/audit 三种发布 Profile。 |
| v1.8 | 2026-08-06 | S17-R1 Runtime Steering 基础切片：新增 InterventionStore（append-only `interventions.jsonl`、模块级路径锁、严格重放与幂等）、`SteeringSnapshot`、`TaskState.active_run_id` 与最小 run 生命周期；ContextBuilder 注入当前 run 的持续 Steering（PENDING/DELIVERED/CONSUMED 全部 effective，不被预算静默裁剪）；AgentLoop 每轮生成 Snapshot、注入 Context、Provider 前 `mark_delivered`。并发线性化/Prepare—Commit—Apply/Approval 绑定/TUI 接入为后续 S17-R2+；单进程 writer 限制。 |
| v1.9 | 2026-08-06 | S17-R2 revision 并发线性化：新增 `commit_action` 与幂等 ledger `action_commits.jsonl`（与 `submit` 争用同一路径锁）；每个 Action 绑定 `SteeringSnapshot.revision`，Steering 先提交则旧 Action `REPLAN`、Action 先提交则 `COMMITTED`；AgentLoop 在 Provider 前（mark_delivered STALE）、Provider 返回后（revision 变化）与 commit 门（REPLAN）三处丢弃陈旧输出，均不消耗 recovery budget。`acknowledge`（CONSUMED）标记与 Prepare—Commit—Apply 大重构留给 S17-R3；Approval 绑定留给 S17-R4。 |
| v2.0 | 2026-08-06 | S17-R3 Steering 确认：AgentLoop 在确认成功 apply 的 choke point（工具派发成功、FINISH_PHASE 成功）调用 `_acknowledge_steering` 把该 Action 实际处理的 `delivery_sequences` 标为 CONSUMED；policy 拒绝、recovery 拒绝、parse 失败、REPLAN、ASK_USER 一律不确认；CONSUMED 仍持续注入 effective。 |
| v2.1 | 2026-08-06 | S17-TUI 运行中 Steering 入口：有 active task 的普通文本均可路由为 `STEER`，不依赖 TUI busy/PauseToken/request 快照；UI 线程通过 `InterventionService` 写入当前 `active_run_id` 的 InterventionStore，已有 Worker 时不启动第二个 mutation Worker，Worker 结束且 run 仍有效时自动 resume；`WAITING_INPUT` 和 `WAITING_APPROVAL` 既有优先级保持不变。 |
| v2.0 | 2026-08-06 | S17-R3 Steering 确认：AgentLoop 在确认成功 apply 的 choke point（工具派发成功、FINISH_PHASE 成功）调用 `_acknowledge_steering` 把该 Action 实际处理的 `delivery_sequences` 标为 CONSUMED（复用 `mark_consumed`，写 `intervention_consumed` trace）；policy 拒绝/recovery 拒绝/parse 失败/REPLAN/ASK_USER 一律不确认；确认为 best-effort，失败不破坏已 apply 状态；CONSUMED 仍持续注入 `effective`，仅从 `awaiting_acknowledgement` 移除。全量 Prepare—Commit—Apply 副作用搬迁与 Approval 绑定（R4）留待后续。 |
...
7. **知识可沉淀**
   deliver 阶段必须把结构化过程证据编译为提交包、学习包和审计包；七份阶段 Markdown 是可复习、可迁移且保留学生笔记的渲染视图。
...
│   ├── TaskController
│   ├── AgentLoop
│   ├── ChangeInspectionService
│   ├── CheckpointInspectionService
...
└── Persistence Layer
    ...
    ├── checkpoint_queries
    ├── learning evidence / traceability
    ├── delivery evidence
    └── submission / learning / audit exports
```
...
```text
ProviderAdapter
  ├─ json_object compatibility mode
  └─ strict json_schema mode
```

- `json_object`（默认）：兼容回退，Action Schema 嵌入 prompt。
- `json_schema`：当服务端支持时，将 Action Schema 放入 API `response_format`，
  使用严格 JSON Schema 约束模型输出。
...
> Prompt 只能引导模型，不能替代 ToolPolicy、PathPolicy、Router、Checkpoint
> 和 Approval 等确定性机制。
...
| code    | `SPEC.md`、`PLAN.md`、相关代码片段、允许修改范围、protected patterns          |
| test    | `PLAN.md`、changed files、测试命令、最近 checkpoint                       |
| review  | `SPEC.md`、`PLAN.md`、`TEST_REPORT.md`、changed files、checkpoint 信息 |
| deliver | 经校验的 learning evidence、traceability、最新测试/Build/Diff 状态和必要阶段摘要 |
...
5. deliver phase 必须看到经校验的学习证据、追踪矩阵和最新测试/Build/Diff 状态；不得加载完整 trace 后自由总结。
...
9. 输出为纯数据字典：`task_id`、`phase`、`goal`、`sections`、`runtime_state`、
   `phase_gate`、`context_risks` 与 `truncation`；ContextBuilder 不写入 state 或
   trace，副作用由后续 AgentLoop 集成负责。
10. 可选项目文档读取失败只形成风险；阶段必需产物、当前 task checkpoint 或 trace 身份无效时
    通过结构化错误 fail-closed。所有注入文本沿用文件读取脱敏规则，且不跟随链接。
11. `runtime_state` 暴露 `status`、`current_phase`、`artifacts`、
    `source_edits_this_phase`、`latest_test_status`、`test_status_consumed`、
    `retry_budget_remaining`、`rollback_required` 和 `rollback_done`，供模型判断
    是否应继续调用工具、修复错误或结束阶段。
12. `phase_gate` 由确定性 `build_phase_gate()` 生成，包含 `can_finish` 和各
    `requirement` 的满足状态；ToolPolicy 与 ContextBuilder 消费同一来源，不存在
    重复的阶段完成判断逻辑。
13. 工具目录由 PromptBuilder 根据阶段过滤并注入，ContextBuilder 不再重复注入
    `allowed_tools`。

#### 6.5.1 Runtime Steering 注入（S17-R1）

Runtime Steering 让用户在 Agent 运行过程中提交新要求，并作为高优先级运行时约束持续进入后续每轮 Context。基础切片的机制如下：

1. 事实来源是 append-only 的 `.hancode/tasks/<task-id>/interventions.jsonl`，由 `InterventionStore` 维护。事件为 `submitted`/`delivered`/`consumed`；`InterventionRecord` 是重放投影，`content` 只保存在 `submitted` 事件且经 `redact_text` 脱敏，纯敏感输入被拒绝。
2. Steering 是持续运行约束：当前 `run_id` 下 `PENDING`/`DELIVERED`/`CONSUMED` 全部持续注入，直到该 run 结束；`CONSUMED` 仅表示某个 Action 已确认处理过，不代表失效。
3. `AgentLoop` 每轮通过 `prepare_context()` 生成不可变 `SteeringSnapshot`，显式传入 `ContextBuilder.build(user_interventions=..., intervention_revision=...)`；**ContextBuilder 不直接读取 Store**。
4. Context 输出新增 `user_interventions.{revision, effective, awaiting_acknowledgement}`（按 sequence 升序，正文脱敏）。较大 sequence 冲突时覆盖较小 sequence，但不绕过系统规则、ToolPolicy、Approval、Checkpoint 或 Phase Gate。
5. Steering 不参与预算裁剪：预算不足时返回结构化错误 `intervention_context_budget_exceeded`，绝不静默删除或截断 Steering。
6. run identity：`TaskState.active_run_id` 由 `/run` 创建并持久化、`/resume` 复用、终态清除；不同 run 使用同一 task-global sequence 空间，但 Context 只加载当前 run 的记录。
7. 并发线性化（S17-R2）：每个 Action 绑定生成它时的 `SteeringSnapshot.revision`，执行前经 `commit_action()` 与 Steering `submit` 争用同一路径锁——Steering 先提交则旧 Action `REPLAN` 且无副作用，Action 先提交则 `COMMITTED`、后到 Steering 在下一轮生效；幂等 ledger `action_commits.jsonl` 按 `commit_key` 保证 crash-retry 返回首次结果。AgentLoop 在 Provider 前（`mark_delivered` 返回 STALE）、Provider 返回后（`current_revision` 变化）与 commit 门（`REPLAN`）三处丢弃陈旧输出，均不消耗 recovery budget、不写旧 parse failure、不派发工具。
8. Steering 确认（S17-R3）：AgentLoop 在确认成功 apply 的 choke point（工具派发成功、FINISH_PHASE 成功）调用 `_acknowledge_steering`，把该 Action 实际观察到的 `snapshot.delivery_sequences` 标为 CONSUMED（复用 `mark_consumed`，写 `intervention_consumed` trace）。policy 拒绝、recovery 拒绝、parse 失败、REPLAN、ASK_USER 等待都不会到达确认点，因此永不误确认；确认为 best-effort 审计元数据，`mark_consumed` 失败不破坏已 apply 的 Action。CONSUMED 记录仍持续注入 Context 的 `effective`（符合"CONSUMED 不代表失效"），仅从 `awaiting_acknowledgement` 移除。
9. TUI 常驻输入（S17-TUI-R2/R4）：composer 始终接收文本；有 active task 且不是 `WAITING_INPUT` 时，普通文本由 UI 线程同步调用 `InterventionService.submit` 写入当前 run 的 `interventions.jsonl`，资格由持久化 `active_run_id` 校验，不依赖 busy/PauseToken/request identity 快照。已有 mutation Worker 时不创建第二个 Worker；Worker 已结束但 run 仍有效时自动 resume。`WAITING_INPUT` 仍优先作为回答；`WAITING_APPROVAL` 普通文本写入 Steering 并自动 resume，但 `/approve`/`/reject` 仍必须显式输入；`PAUSED` 不自动恢复。
10. Approval 绑定与失效（S17-R4）：新 manifest 保存 `run_id` 与 `steering_revision_at_request`，恢复时校验二者；PENDING/APPROVED 漂移按 manifest EXPIRED、清理 pending pointer、恢复 RUNNING、写 `approval_expired_by_intervention` Trace 并重新规划；旧未绑定 PENDING/APPROVED 显式过期，旧 EXECUTING/CONSUMED fail-closed。批准 Action 在 EXECUTING、Checkpoint、Tool dispatch 前经独立 Approval `commit_action()` gate，REPLAN 不执行旧 Action；非 checkpoint Action 也必须先进入 EXECUTING。
11. 后续边界：全量 Prepare—Commit—Apply 副作用边界搬迁（把所有旧决策副作用移到统一提交门之后）本轮未做；CANCEL/STOP 与跨进程文件锁也不在 MVP。单进程 writer：不支持多个 HanCode 进程同时写同一任务。

Provider Prompt 契约（S17-P0）：`providers/prompt_contract.py` 的 `RUNTIME_STEERING_CONTRACT` 同时进入文本 `BASE_SYSTEM_CONTRACT` 与 native tool-calling system message，明确 `task_context.user_interventions.effective` 在当前 run 的持续有效性、sequence 覆盖关系和安全边界；不改变 Action JSON 或 Function Tool 输出格式。
...
| `final`       | 任务收尾（模型主动结束）                     | 交由 ResultBuilder 生成最终结果                                          |
...
7. 测试失败后是否进入 review。
8. deliver 的结构化证据、硬门禁、七份阶段视图和发布 manifest 是否一致。
...
deliver phase 不应写入 SOURCE 区，只能收集/校验证据、渲染 ARTIFACT 视图并发布 Profile。
...
| deliver | DeliveryValidator 的硬门禁全部通过，七份阶段视图已渲染，发布 manifest 与 evidence digest 一致 | "交付证据无效或发布未完成" |
...
class ToolPolicy:
    def validate_finish_phase(self, action, phase, state, config) -> Decision:
        checks = {
            ...
            Phase.DELIVER: lambda: delivery_validator.is_publishable(state),
        }
        ok = checks[phase]()
...
| `record_*` + `ArtifactRenderer` | medium | code, test, review, deliver | 先写结构化证据，再渲染 `IMPLEMENTATION.md`、`TEST_REPORT.md`、`REVIEW.md`、`KNOWLEDGE.md`、`DELIVERABLES.md` |
...
5. 确定性报告类文件（IMPLEMENTATION/TEST_REPORT/REVIEW/KNOWLEDGE/DELIVERABLES）只能由 ArtifactRenderer 根据已验证证据生成；通用 write_file 不提供兜底路径。
```

**write_file 与结构化证据/Renderer 的职责边界**

为消除“同一文件两条写入路径、风险等级还不同”的歧义（v1.0 中 write_file=high、write_report=medium）：

```text
确定性交付 Markdown（IMPLEMENTATION/TEST_REPORT/REVIEW/KNOWLEDGE/DELIVERABLES）→ 结构化 record 工具 / Collector → Validator → ArtifactRenderer。
...
T17 提供函数式 `create_checkpoint(task_root, files, reason, *, created_at=None)` 与 `commit_checkpoint(task_root, checkpoint_id)`：创建函数在 code phase 对 SOURCE 区目标建立 `pending` manifest，现有文件保存 before snapshot/hash，不存在的 SOURCE 目标记录为 `action="create"`；提交函数验证同 task 的 manifest、before snapshot/hash 和当前目标文件后，补 after hash 并原子更新为 `committed`。

T18 提供 `rollback_last_checkpoint(task_root) -> RollbackResult`。它只在 task state 一致且处于 review phase 时，恢复 `state.latest_checkpoint` 指向的同 task、同 project、code phase、`committed` 且可用 manifest。预检先完成 manifest/快照、SOURCE 路径、symlink/junction、目标 current hash 与 after hash 的全部校验；外部改动或无法验证当前内容均以 `rollback_conflict` 阻断，零业务文件写入，不提供 force / confirm。`modify` 使用同目录独占随机临时文件后 replace，`create` 删除 checkpoint 后创建的目标。

```text
checkpoint_id：只使用 state.checkpoint_seq + 1，格式 ckpt-NNN。
目录：快照和 manifest 先写 .ckpt-NNN.tmp，完成后 rename 为 checkpoints/ckpt-NNN/。
边界：task root、checkpoints 根、checkpoint/temporary 目录、manifest 与 files/ 均不得解析到 task workspace 外；拒绝 symlink/junction 外链。
安全：只接受 PathClassifier 的 SOURCE；不快照 PROTECTED 文件；reason 按敏感赋值/Bearer 模式脱敏后才写 manifest 和 trace。
审计：创建写 checkpoint_created，提交写 checkpoint_committed；state 或 trace 失败时执行补偿，补偿失败返回结构化错误。
```

T18 成功后将 manifest 标记为 `rolled_back`、关闭 `rollback_available`，保留 `latest_checkpoint`、checkpoint seq 与 retry budget，复位 code/test/review 完成标记、测试状态和本 phase 编辑计数，并记录 `rollback_started` / `rollback_performed`。文件、manifest、state 或 trace 任一阶段失败都补偿已完成步骤；补偿失败将任务标记为 `inconsistent`。T21-R1 在此基础上接入 FileTools、AgentLoop 和启动 reconcile；checkpoint pruning、多 checkpoint 事务和外部攻击者级 TOCTOU 防护仍不在本任务范围。
...
rolled_back→ 已执行过回退
aborted    → pending 已确认未生效，或已按显式恢复策略恢复 before snapshot，不计入 rollback 链
```
...
```text
发现 status == "pending" 的 checkpoint：
  → 所有目标仍等于 before 状态：自动标记 aborted，不写业务文件；
  → 任一目标已变化：普通启动保存 inconsistent + rollback_required，写 checkpoint_recovery_required 并停止；
  → 仅调用方显式传入 resume=True，才恢复已预检的 before snapshot，再标记 aborted；
  → 恢复成功后 state.status = blocked、inconsistent = false、rollback_required = false，随后由本次显式 resume 转为 running；
  → manifest、snapshot、hash、symlink/junction 无法验证时，零业务文件写入，返回结构化错误。
```
...
rollback 前，对每个目标文件计算 current_sha256：
  ...
  current != after_sha256  → 文件被外部（学生手动 / 其他进程）改过：
        不静默覆盖；T18 固定中止 rollback，返回 rollback_conflict，
        不提供用户确认或 force 覆盖。
```
...
2. 达到上限时返回 `checkpoint_limit_exceeded`，拒绝新增，不自动清理旧 checkpoint。
3. pruning、多 checkpoint 事务和批量编辑不在 T21-R1 范围内，`latest_checkpoint` 与 `checkpoint_seq` 仍保持单调、可审计。
```

rollback 成功后的副作用：

```text
1. manifest 状态转为 rolled_back、rollback_available=false，重复恢复被阻断。
2. state 记录 rollback_done=true、rollback_required=false，保留 latest_checkpoint、checkpoint seq 与 retry budget。
3. latest_test_status 重置为 none、test_status_consumed=false、source_edits_this_phase=0；code/test/review 的 phase_completed 均复位。
4. 写入 rollback_started 与最终 rollback_performed trace，并记录 checkpoint_id 与最终 restored_files。
5. 控制流保持在 review，由后续 review 决定 blocked、deliver 或重新进入 code。
6. 不得在 rollback 后跳过 review 直接大范围继续修改；inconsistent task 不得执行 rollback。
```
...
T16 提供函数式 `append_trace(task_root, ...) -> TraceEvent`。调用方提交事件字段，TraceLogger 负责生成编号、脱敏并追加到 task root 的 `trace.jsonl`；T16 不提前实现 AgentLoop 集成、并发 writer lock、`fsync` 或崩溃后的半行恢复。

```text
seq：从 1 开始，追加前逐行验证既有 JSONL 的连续序号。
event_id：固定为 evt-{seq:06d}，例如 evt-000001；历史编号异常时拒绝追加。
append-only：只追加，不重写已有事件。
task isolation：task root 必须位于 .hancode/tasks/<task_id>，且存在有效 project.json；历史事件 task_id 必须一致。
write failure：返回 trace_write_error，调用方必须将其视为 fail-closed 信号，阻止后续高风险动作。
```

事件载荷中的敏感字段和键值文本统一脱敏；规范化名称以 `content`、`output`、`stdout`、`stderr`、`body`、`text` 开头或结尾的内容字段只记录 `[CONTENT_OMITTED]` 与长度。JSON 编码失败、既有 trace 损坏、非 Mapping payload 或工具审计字段不完整时，返回结构化 `HanCodeError`，不回显底层异常。
...
T20 的 `Observation` 是冻结数据结构，固定包含 `kind`、`success`、`phase`、`summary`、
`next_action_hint`、可选 `failure_category` 与只读 `details`。摘要、建议和 details 必须在构造前
脱敏（包括裸 `Bearer` token 与敏感键）；工具结果必须由 AgentLoop 显式传入当前 phase；
`ToolResult.timed_out` 是超时的唯一结构化信号，不依赖错误文案猜测。

分类规则（按优先级自上而下匹配，命中即停；`NONE` 仅在其余类别均不命中且退出码为 `0` 时成立）：
...
ResultBuilder 读取结构化 evidence 和 traceability，不解析 Markdown 反推机器状态。`REVIEW.md` 至少渲染以下需求追踪表：

```text
requirement_id | decision_ids | change_ids | test_ids | status | evidence | risk
```

其中 `status` 只能是 `covered`、`partial`、`missing`、`untested`。核心需求只有同时存在当前有效 `ChangeEvidence` 与 `TestAttemptEvidence` 才能判为 covered。

`KNOWLEDGE.md` 由 `KnowledgeCard` 渲染。每张卡必须包含问题、上下文、原则、解法、证据引用、适用/不适用场景、常见错误和迁移练习；`evidence_refs` 必须指向当前 task 中存在且 digest 有效的 `R-* / D-* / C-* / T-* / F-*` 或受控审计证据。

所有阶段 Markdown 使用 generated/student 分区。Renderer 只替换 `<!-- hancode:generated:start/end -->` 内部内容；学生的理解、疑问和教师/同伴反馈不进入机器门禁，也不被自动覆盖。
...
| `ARTIFACT`     | 阶段产物区；SPEC/PLAN 可走 `write_file`，其余学习/交付视图由结构化 record 工具与 Renderer 写入 | `.hancode/tasks/<task>/`（SPEC/PLAN/IMPLEMENTATION/TEST_REPORT/REVIEW/...） |
...
| 工具            | 允许写入的区                    | 说明                                       |
| ------------- | ------------------------- | ---------------------------------------- |
| 结构化 `record_*` + `ArtifactRenderer` | ARTIFACT | 确定性学习/交付视图的唯一写入路径 |
| `write_file`  | ARTIFACT(spec/plan) / SOURCE(code) | 由当前 phase + Zone 联合判定；不得写确定性交付视图 |
...
```json
{"event_id":"evt-000001","seq":1,"event_type":"task_started","task_id":"task-001","phase":"spec","status":"running","action":null,"observation":null,"error_summary":null,"state_transition":{"status":["created","running"]}}
{"event_id":"evt-000002","seq":2,"event_type":"phase_started","task_id":"task-001","phase":"spec","status":"running","action":null,"observation":null,"error_summary":null,"state_transition":{"phase":[null,"spec"]}}
{"event_id":"evt-000003","seq":3,"event_type":"tool_called","task_id":"task-001","phase":"spec","status":"running","action":{"tool_name":"write_file","args":{"path":"SPEC.md"},"reason":"Create task specification.","policy_decision":{"allowed":true,"message":"Action is allowed.","phase":"spec","denied_rule":null,"suggested_fix":""}},"observation":null,"error_summary":null,"state_transition":null}
{"event_id":"evt-000004","seq":4,"event_type":"tool_completed","task_id":"task-001","phase":"spec","status":"succeeded","action":{"tool_name":"write_file","args":{"path":"SPEC.md"},"reason":"Create task specification.","policy_decision":{"allowed":true,"message":"Action is allowed.","phase":"spec","denied_rule":null,"suggested_fix":""}},"observation":{"summary":"[CONTENT_OMITTED]","char_count":128},"error_summary":null,"state_transition":{"artifacts.SPEC.md":[false,true]}}
{"event_id":"evt-000005","seq":5,"event_type":"phase_started","task_id":"task-001","phase":"code","status":"running","action":null,"observation":null,"error_summary":null,"state_transition":{"phase":["plan","code"]}}
{"event_id":"evt-000006","seq":6,"event_type":"checkpoint_created","task_id":"task-001","phase":"code","status":"succeeded","action":null,"observation":null,"error_summary":null,"state_transition":{"latest_checkpoint":[null,"ckpt-001"]}}
{"event_id":"evt-000007","seq":7,"event_type":"tool_called","task_id":"task-001","phase":"code","status":"running","action":{"tool_name":"edit_file","args":{"path":"src/main.py"},"reason":"Update source code.","policy_decision":{"allowed":true,"message":"Action is allowed.","phase":"code","denied_rule":null,"suggested_fix":""}},"observation":null,"error_summary":null,"state_transition":null}
{"event_id":"evt-000008","seq":8,"event_type":"test_completed","task_id":"task-001","phase":"test","status":"failed","action":null,"observation":{"summary":"[CONTENT_OMITTED]","char_count":256},"error_summary":"Test command failed.","state_transition":{"latest_test_status":["none","failed"]}}
{"event_id":"evt-000009","seq":9,"event_type":"rollback_performed","task_id":"task-001","phase":"review","status":"succeeded","action":null,"observation":null,"error_summary":null,"state_transition":{"rollback_done":[false,true]}}
```
...
| 结构化 `record_*`          | medium | 写入经 schema 校验的学习/交付证据 |

### 9.3 Phase 工具权限

| Phase   | 允许工具                                                                                              |
| ------- | ------------------------------------------------------------------------------------------------- |
| spec    | `read_file`, `list_files`, `write_file`, `search_text`                                            |
| plan    | `read_file`, `write_file`, `search_text`, `list_files`                                            |
| code    | `read_file`, `list_files`, `search_text`, `edit_file`, `write_file`, `run_tests`                  |
| test    | `read_file`, `run_tests`；测试证据由 Harness 自动记录并渲染                                      |
| review  | `read_file`, `list_files`, `search_text`, `run_tests`, `rollback_last_checkpoint`, `record_remediation`, `record_review` |
| deliver | `read_file`, `get_diff`, `record_knowledge`, `finish_phase`；发布由 DeliveryPipeline 执行          |
...
3. 校验当前 task checkpoint 的 manifest、身份、路径、快照和 before hash；无法验证时 fail-closed。
4. 处理 pending checkpoint：未发生源文件变化时自动 abort；发生变化时普通启动保存 `inconsistent + rollback_required`，只允许显式 `resume=True` 恢复（见 §6.9.1）。
5. 若 state.json 损坏或缺字段，进入 blocked 或 failed，提示人工修复，不从 history.jsonl 静默重建状态。
6. reconcile 结果写 trace（state_reconciled / state_inconsistent）；pending 路径另写 checkpoint_aborted、checkpoint_recovery_required 或 checkpoint_recovery_failed。
```

> 实现现状（T21-R1）：`FilesystemStateStore.reconcile(recover_pending=False)` 先执行 checkpoint 对账，再执行 artifact 漂移检查。未修改 pending 自动标记 `aborted` 并回指最近可 rollback 的 committed checkpoint；已变化 pending 在普通启动下 fail-closed，显式 `recover_pending=True` 才恢复经过预检的 before snapshot。恢复成功保留 `checkpoint_seq` 单调递增，并以 `blocked` 状态等待本次 `resume=True` 转为 running；校验或补偿失败均返回结构化错误并尽量写审计 trace。
...
`run_tests` 只接受项目配置中的命令字符串，不让 LLM 拼接命令或调用通用 shell；实际测试框架仍可能加载仓库里的测试文件。code 阶段 LLM 能写新文件，因此可写 `test_evil.py`（`import os; os.system(...)`），让合法的测试入口成为任意代码执行跳板。
...
3. 缓解二：测试命令在受限子进程中运行——`shlex.split` 固定 argv、`shell=False`、
   超时、工作目录限定在 project root、捕获输出；不自动安装依赖，也不提供网络沙箱承诺。
...
构造时、每次记录 context 时和每次返回 action 时均执行深拷贝；因此调用方后续修改预设 action、传入 context 或返回 action，均不会影响 MockLLM 的内部状态。`contexts` 公开只读历史时返回新的 tuple，并为其中每条 context 返回深拷贝快照，避免调用方通过历史视图修改内部记录。

MockLLM 不以 `blocked` 或 `final` dict 表示序列耗尽；AgentLoop 捕获 `MockLLMExhausted` 后，固定进入并返回 `blocked`。`final` 仍是模型可主动提出的合法 Action。
...
test_artifact_renderer_is_only_path_for_deterministic_reports
...
| 最终学习发布             | DeliveryPipeline、Learning Evidence、Traceability、Publisher | 生成七份学习视图与三类发布包 |
...
6. rollback 结果会反馈回灌。
7. deliver 必须从已验证证据生成七份阶段视图并发布所选 Profile；不能以空 Markdown 绕过硬门禁。
8. trace 能记录全过程。
9. MockLLM 能离线复现流程。
...
9. deliver phase 运行五步发布管线，生成七份阶段视图及 submission / learning / audit Profile。
...
flowchart TB
    ...
    Registry --> RecordReview["record_review"]
    Registry --> RecordKnowledge["record_knowledge"]
    ...
    RecordReview --> DeliveryPipeline["DeliveryPipeline"]
    RecordKnowledge --> DeliveryPipeline
...
flowchart LR
    ...
    Deliver --> Collector["LearningEvidenceCollector"]
    Collector --> Validator["DeliveryValidator"]
    Validator --> Renderer["ArtifactRenderer"]
    Renderer --> LearningDocs["Seven Learning Markdown Views"]
    Validator --> Publisher["DeliveryPublisher"]
    Publisher --> Profiles["Submission / Learning / Audit"]
```
...
flowchart TB
    ...
    Code --> ChangeEvidence["ChangeEvidence C-*"]
    ChangeEvidence --> Implementation["IMPLEMENTATION.md"]

    ChangedFiles --> Test["test phase"]
    Test --> Attempts["TestAttemptEvidence T-*"]
    Attempts --> TestReport["TEST_REPORT.md (all attempts)"]
    ...
    Checkpoint --> Review
    ChangedFiles --> Review
    Review --> Traceability["Requirement Traceability"]
    Traceability --> ReviewReport["REVIEW.md"]
    ...
    ChangeEvidence --> Deliver
    Attempts --> Deliver
    Traceability --> Deliver
    Deliver --> Pipeline["Collect → Validate → Synthesize → Reflect → Publish"]
    Pipeline --> Deliverables["DELIVERABLES.md Index"]
    Pipeline --> Knowledge["KNOWLEDGE.md Cards"]
    Pipeline --> Profiles["Submission / Learning / Audit"]
...
HanCode 不直接提交整个 `.hancode/`，而由 `DeliveryPublisher` 按 Profile 从已经验证的 manifest 发布：

```bash
hancode export --task task-001 --profile submission --out export/submission/
hancode export --task task-001 --profile learning --out export/learning/
hancode export --task task-001 --profile audit --out export/audit/
```

- `submission`：README、索引型 DELIVERABLES、课程提交源码和 delivery manifest；排除 `.hancode` 内部运行状态。
- `learning`：LEARNING_INDEX、SPEC、PLAN、IMPLEMENTATION、TEST_REPORT、REVIEW、KNOWLEDGE 和 final.diff，供学生复盘；`DELIVERABLES.md` 由 submission 提供。
- `audit`：manifest、结构化 evidence、脱敏 trace、checkpoint manifest 与 requirement traceability；不包含 checkpoint 原始快照。

三个 Profile 均使用 staging 目录、同文件系统原子发布、防覆盖、逐级 link/reparse 检查和敏感内容扫描。Runtime Memory 的 events/index/blobs、凭据、本地 provider 配置、原始 checkpoint 文件及未脱敏 Trace 不得进入任何 Profile。
...
## S6 Provider 协议边界（2026-07-29）

```text
ContextBuilder
  -> PromptBuilder (text: hancode-action-v2 | native: hancode-tool-v1)
  -> OpenAICompatibleProvider
       -> transport retry (独立于协议重试)
       -> Decoder + SchemaValidator + StrictSchemaProjection/Normalizer
       -> ProviderError(protocol_retryable)
  -> AgentLoop (配置化 provider_protocol_retries)
       -> parse_action -> ToolPolicy -> ApprovalPolicy -> ToolRegistry
```

`ProviderToolDefinition` 保存 request schema、原始 ToolSpec args schema 与 strict projection，禁止在投影中修改 ToolSpec。Auto Provider 缓存最近接受的 effective mode，只从 strict 向 non-strict、JSON Schema、JSON object 移动。其唯一事件是 `ProviderEvent(kind="mode_fallback")`；Engine 将其持久化为 `provider_mode_fallback`，且不吞掉 sink 的异常，使现有 AgentLoop 一致性边界接管失败。

## S8 项目配置中心（2026-07-29）

```text
PROJECT_CONFIG_DEFAULT_ITEMS
  -> init_project_workspace（新项目完整模板）
  -> ConfigLoader（运行时不可变 HanCodeConfig）
  -> ConfigService（规范化草稿、同一校验、原子替换）
  -> ConfigScreen
       -> ConfigTuiApp（hancode config setup / init --configure）
       -> HanCodeTuiApp（/config / Command Palette）
```

配置默认值与字段顺序由不依赖 Storage/TUI 的 Core 模块统一提供，避免 Workspace 初始化、
ConfigLoader 和页面分别维护默认值。ConfigService 是唯一配置写入入口：先对内存候选执行
现有 ConfigLoader 校验，再在 `.hancode` 内写临时文件并通过 `os.replace` 替换。

配置 TUI 编辑非敏感项目元数据，并通过独立的 AuthService 通道管理 API Key。隐藏输入值只传给
CredentialProvider 并写入系统 Keyring，不经过 ConfigService，也不进入 `project.json`、日志或
Trace；TUI 只持有 CredentialStatus 的来源与末四位掩码。环境变量和 `.env` 来源保持只读。
任务运行中不允许修改配置，已有任务状态和 `retry_budget_remaining` 不因配置保存而被重写。

## S10-R1 测试失败诊断与自主修复闭环（2026-07-31）

```text
run_tests
  -> FeedbackReport
  -> TestFailureRecord（脱敏摘要、策略/命令绑定、规范化 fingerprint）
  -> REVIEW / record_remediation
       -> modify_source / modify_test -> CODE + planned_paths 约束
       -> replace_test_strategy       -> CODE + record_test_strategy
       -> rerun_for_diagnosis         -> TEST（每个失败最多一次）
       -> request_input               -> WAITING_INPUT
       -> rollback                    -> 既有 Checkpoint/Rollback
  -> 原登记策略 TEST
       -> passed  -> 最终 REVIEW / record_review
       -> changed -> 新 failure，继续诊断
       -> repeated twice -> WAITING_INPUT 或 BLOCKED
```

`test_failure.json` 与 `test_remediation.json` 是任务级内部机器记录，不属于交付 artifact，也不能由通用文件工具生成。两者使用 schema v1、task identity、SHA-256 digest、大小上限、链接拒绝和同目录原子替换；历史只进入受限 Trace。

`TaskState` 保持 schema v1，并增加四个向后兼容可选字段：`latest_test_failure_digest`、`latest_remediation_digest`、`test_attempt_seq`、`remediation_applied`。旧失败只在显式 resume 时由现有 `TEST_REPORT.md` 惰性建档，禁止手工修补 state。

自主修复预算在修复真正应用时扣减：一次 `modify_source`、`modify_test` 或 `replace_test_strategy` 只扣一次；诊断复测和人工输入不扣。源码写入仍经过 PathClassifier、ToolPolicy、Checkpoint 和 Approval，且活动修复期间必须命中 `planned_paths`。

Windows runner 以 bytes 捕获输出，按 BOM/NUL、UTF-8 和系统编码安全解码。WSL launcher、Git Bash `Win32 error 5`、`WinError 5`、`PermissionError` 与 `FileNotFoundError` 统一分类为 `environment_error`；只保留稳定异常类型及 errno/winerror，不保存敏感路径。

## S12-R1：TUI 协作式暂停边界

```text
TUI /pause -> request-id 绑定的 PauseToken -> Controller -> OperationExecutor
    -> TaskService -> Engine -> AgentLoop 安全点
    -> state.json: PAUSED -> trace: run_paused -> TUI 解除 busy

TUI /resume -> 同一 RUN_TASK 路径 -> state.json: RUNNING -> trace: run_resumed
```

`PauseToken` 不属于 `TuiOperation`，因此暂停不是第二个 mutation，也不会与现有单 mutation Worker 竞争。Worker 结果经原 request-id gate 接受后才清理对应 token，迟到结果不能影响后续运行。暂停不向 Provider、ToolRegistry、测试 runner 或 Textual Worker 发 cancel；这些边界完成后才进入下一安全点。

## S13 Task-scoped Runtime Memory（2026-08-04）

Task Runtime Memory 是独立于 `state.json`、Trace 和交付物的 task 内部运行时存储：

```text
ToolResult
  -> FilesystemMemoryStore.record_tool_result()
  -> memory/events.jsonl（权威 append-only 日志）
  -> memory/index.json（可重建派生索引）
  -> memory/blobs/<sha256>.txt|json（内容寻址、脱敏后正文）
```

`MemoryContextPacker` 在每轮 ContextBuilder 预算内注入最近摘要、当前有效文件索引和少量热点正文。
`memory_read` 与 `memory_search` 只接受 Registry 绑定的当前 task；读取 stale 历史必须显示失效原因，
搜索默认排除 stale。文件指纹变化、write/edit 和 rollback 都通过 generation/invalidation 重放更新有效性，
因此旧 blob 可以审计和按需读取，但不会重新成为当前文件。

运行时记忆配置沿用 `PROJECT_CONFIG_DEFAULT_ITEMS` 的唯一默认值真源：

| 配置 | 默认值 | 作用 |
| --- | ---: | --- |
| `max_memory_blob_bytes` | `1_048_576` | 单个 UTF-8 blob 上限 |
| `max_memory_task_bytes` | `33_554_432` | 单 task events/index/blobs 实际占用上限 |
| `max_memory_recent_events` | `8` | 自动注入的最近摘要数量 |
| `max_memory_file_entries` | `32` | 自动注入的有效文件索引数量 |
| `max_memory_hot_contents` | `2` | 自动注入的热点正文数量 |

S14 的三个发布 Profile 均不包含 `memory/`；因此 Runtime Memory 不会进入提交、学习或审计包。
S13-R5 的离线 MockLLM Demo 通过“初始读取 → Memory Tool → 新写入 stale → 测试失败与
rollback → 重新读取历史”验证跨 AgentLoop 实例恢复，不依赖网络、真实 LLM 或凭据。

## S14 Deliver 学习发布架构（2026-08-06）

### S14.1 定位与权威边界

Deliver 不是“完成证明生成器”，而是将开发过程中的结构化证据编译成提交包、学习包和审计包的发布阶段：

```text
结构化证据 = Source of Truth
Markdown = 面向学生的可再生视图
Export Profile = 面向具体受众的发布结果
```

`state.json` 继续作为任务生命周期唯一权威；`learning/events.jsonl` 是学习事件的 append-only 权威日志；`learning/evidence.json` 与 `learning/traceability.json` 是可由合法事件前缀重建的当前证据/关系视图。Markdown、TUI 文案和 LLM 总结均不得反向推进状态或修补证据。

Task Runtime Memory 与 Learning Evidence 职责不同：Memory 为 Agent 跨轮恢复上下文，允许保留 stale 历史；Learning Evidence 为最终学习/交付结论提供可校验依据。两者不共享 Store、schema、配额或导出边界。

### S14.2 稳定证据 ID 与学习链

| 前缀 | 实体 | 典型来源 |
| --- | --- | --- |
| `R-*` | RequirementEvidence | SPEC 需求理解 |
| `D-*` | DecisionEvidence | PLAN 候选方案与选择理由 |
| `P-*` | Plan Step | PLAN 实现步骤 |
| `C-*` | ChangeEvidence | checkpoint、Diff、changed files |
| `T-*` | TestAttemptEvidence | 每次真实 run_tests 结果 |
| `F-*` | FailureEvidence | 确定性失败分类与诊断 |
| `K-*` | KnowledgeCard | 基于已有证据提炼的迁移知识 |
...
所有引用先做 task identity、schema、digest、存在性和允许关系校验。LLM 可生成候选解释和 KnowledgeCard 字段，但不能分配事实 ID、制造 Diff/测试/checkpoint，也不能引用不存在的证据。

### S14.3 Learning Event 与领域模型

`learning/events.jsonl` 追加以下事件：
...
测试状态同时保留 `test_attempts[]`、`latest_test_attempt_id` 和 `latest_test_status`。前两者用于学习与审计，最后一个只供 Router/状态摘要使用；新测试结果只追加，不覆盖历史失败。

### S14.4 五步 Delivery Pipeline

```mermaid
flowchart LR
    Sources["SPEC / PLAN / Diff / Checkpoint / Tests / Failure / Review"] --> Collect["Collect"]
    Collect --> Validate["Validate"]
    Validate --> Synthesize["Synthesize"]
    Synthesize --> Reflect["Reflect"]
    Reflect --> Publish["Publish"]
    Publish --> Submission["Submission"]
    Publish --> Learning["Learning"]
    Publish --> Audit["Audit"]
```

1. **Collect / LearningEvidenceCollector**：只从受控工具结果、state、checkpoint manifest、Delivery Evidence 和已验证学习事件收集事实；不让 LLM 重读完整 Trace 生成事实。
2. **Validate / TraceabilityBuilder + DeliveryValidator**：验证核心需求覆盖、证据 ID、失败修复闭环、最新测试/Build/Diff、敏感信息和 task identity。
3. **Synthesize / ArtifactRenderer**：基于证据生成七份 Markdown 的 generated 区域；解释必须带证据引用。
4. **Reflect**：提供学生自测、个人理解、未理解项和教师/同伴反馈；缺失产生 warning，不伪装成学生已经理解。
5. **Publish / DeliveryPublisher**：基于同一份已冻结 evidence digest 原子发布三种 Profile；不得在不同 Profile 中重新计算互相矛盾的完成结论。

`DeliveryPipeline` 只负责编排、错误转换和补偿边界，不继续同时承担 Store、校验、渲染和复制职责。

### S14.5 Markdown 工作簿

| 阶段 | 文件 | 学习问题 |
| --- | --- | --- |
| spec | `SPEC.md` | 我如何理解题目、边界和验收证据？ |
| plan | `PLAN.md` | 为什么选择这个方案，为什么放弃其他方案？ |
| code | `IMPLEMENTATION.md` | 实际改了什么，为什么这样改，如何验证？ |
| test | `TEST_REPORT.md` | 哪些测试失败，如何定位、修复和复测？ |
| review | `REVIEW.md` | 每个需求由哪些设计、实现和测试证据支持？ |
| deliver | `KNOWLEDGE.md` | 哪些原则可迁移，适用和不适用边界是什么？ |
| deliver | `DELIVERABLES.md` | 现在能否提交，如何运行/测试，学习与审计资料在哪里？ |

`DELIVERABLES.md` 只做首页和索引，不复制其他六份文档正文。`TEST_REPORT.md` 展示全部有效 `T-*`；`REVIEW.md` 展示需求追踪矩阵；`KNOWLEDGE.md` 展示 `KnowledgeCard`；`IMPLEMENTATION.md` 的文件、Diff 和 checkpoint 事实由 Harness 确定性生成。
...
Renderer 必须解析并验证唯一、顺序正确、不嵌套的标记；任何歧义都停止重写，避免破坏学生内容。

### S14.6 完成门禁与学习告警

硬门禁失败时不得 `completed`：

- 每个核心需求都有合法覆盖状态、实现证据和测试证据；
- 最新测试通过；配置 Build 时最新 Build 通过；
- KnowledgeCard 的证据引用全部有效；
- 存在失败历史时至少有一条 `F-* → Recovery → C-* → T-*` 完整链；
- 最终 Diff 与 checkpoint 证据属于当前 workspace 状态；
- 发布内容未命中敏感信息，manifest 与 evidence digest 一致。

以下仅进入 `learning_warnings`：

- 没有记录备选设计或舍弃原因；
- 学生未填写反思或自测问题；
- KnowledgeCard 缺少高质量迁移示例；
- 某些修改有“做了什么”但“为什么”证据不足。

保持 `completed / blocked / failed`，不增加“学习不完整”状态。`blocked` 表示证据或前置条件尚可修复，`failed` 只用于不可恢复执行失败；warning 不改变终态。

### S14.7 三种发布 Profile

| Profile | 主要受众 | 必含 | 必须排除 |
| --- | --- | --- | --- |
| submission | 课程提交 | README、DELIVERABLES、source、delivery manifest | `.hancode` 内部状态、Trace、Memory、checkpoint |
| learning | 学生复盘 | LEARNING_INDEX、SPEC、PLAN、IMPLEMENTATION、TEST_REPORT、REVIEW、KNOWLEDGE、final.diff | 凭据、内部 Store、原始 checkpoint |
| audit | 教师/答辩/Harness 验证 | manifest、evidence、sanitized trace、checkpoint manifest、traceability | 原始 checkpoint 文件、未脱敏输出、Runtime Memory blob |

学习包不是课程提交包的超集，审计包也不是 `.hancode/` 的完整复制。Publisher 对每个 Profile 使用显式 allow-list，并在复制前后校验文件摘要。

### S14.8 当前代码映射与演进顺序

| 当前模块 | S14 演进 |
| --- | --- |
| `core/delivery_evidence.py` | 增加 Requirement/Decision/Change/TestAttempt/Failure/Recovery/KnowledgeCard/Traceability 模型 |
| `storage/delivery_evidence.py` | 拆出或扩展 task-bound LearningEvidence Store 与重放校验 |
| `runtime/delivery_pipeline.py` | 收敛为五步编排器，不再独占记录、校验、渲染、状态推进 |
| `record_knowledge` | 从弱结构 KnowledgeItem 改为结构化 KnowledgeCard，并验证 evidence_refs |
| `TEST_REPORT.md` 写入 | 从最新快照改为全部有效 TestAttemptEvidence 的确定性渲染 |
| `export` / `DeliveryService` | 增加 profile 参数和 Profile-specific manifest |

实施依赖顺序固定为：证据模型/Store → IMPLEMENTATION 与学生笔记保护 → 测试历史 → Traceability/KnowledgeCard → Validator/Renderer/Pipeline → 三种 Profile → TUI 反思浏览。不得先通过扩写 Markdown 模板伪装为学习机制已经完成。
