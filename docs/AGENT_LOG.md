...
- 范围：新增 `src/hancode/providers/capability.py`；重构 `src/hancode/providers/openai_compatible.py`（请求构建、协商循环、fallback）；`src/hancode/providers/base.py` 的 `ProviderEvent.from_mode/to_mode` 由 `ProviderActionMode` 放宽为 `str`；`.hancode/project.json` 的 `provider_action_mode` 由 `json_object` 改为 `auto`。Canonical Action 与 `reason` 保持严格不变。
- 设计：引入 `ProviderCapabilityProfile`（action_encoding + strict_tools/tool_choice_required/disable_parallel_tool_calls/strict_json_schema）。单调能力移除状态机：`native strict+required+serial → 去strict → 去parallel → 去tool_choice → native only → json_schema+strict → json_schema → json_object → prompt_json`，每步只减一个能力，配 visited 集 + 最大 transition 数防环。
- 关键改动：(1) `_build_request` 按 profile 决定是否发送 `function.strict`/`tool_choice=required`/`parallel_tool_calls`/`json_schema.strict`，此前均硬编码；(2) 能力错误分类器：参数路径归一化 `tools[N].function.strict → tools.*.function.strict`，综合 `status_code+code+type+param+message`，放宽错误码（`invalid_request_error/unsupported_feature/not_supported/422` 等），strict 请求粗粒度 `param=tools` 先降 non-strict native（P0-6），未知 400 仍 fail-closed；(3) HTTP 200 协议失败（tool_call_missing/empty/invalid 等）同编码重试 1 次后切下一编码（P0-4）；(4) 新增 `prompt_json` 末端兜底（不发 response_format/tools/tool_choice，仅 prompt 内嵌 action 契约，仍走严格 action schema 校验），仅 auto 内部可达，不进配置白名单；(5) fallback trace 改 best-effort，sink 异常不再中断协商（P1-3）。
- 本轮不做：P1-1（protocol retry 从 AgentLoop 迁入 Provider）——改为在 Provider 内部就近处理 200 降级，不动 AgentLoop 现有 `provider_protocol_retries` 语义。
